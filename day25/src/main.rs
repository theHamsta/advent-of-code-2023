...
// Mincut algo from https://github.com/Qiskit/rustworkx/blob/3bc7dc456eb611c9e7cc05122e8b58f4766add2a/rustworkx-core/src/connectivity/min_cut.rs#L121
...
/// );
/// ```
pub fn stoer_wagner_min_cut<G, F, K, E>(graph: G, mut edge_cost: F, min_cost: K) -> MinCut<K, G::NodeId, E>
where
    ...
    K: Copy + Ord + Zero + AddAssign,
{
    ...
    for phase in 0..(graph_with_super_nodes.node_count() - 1) {
        ...
        {
            if min_cut_val.is_none() || Some(cut_w) < min_cut_val && cut_w >= min_cost {
...
fn main() -> anyhow::Result<()> {
    ...
    let mut graph = petgraph::Graph::<String, (), Undirected>::new_undirected();
    ...
    for l in input.lines() {
        ...
        nodes.entry(node.to_string()).or_insert_with(Vec::new);

        let rest = it.next().unwrap();
        for other in rest.split(' ').filter(|o| !o.is_empty()) {
            nodes.entry(other.to_string()).or_insert_with(Vec::new).push(node.to_string());
            //nodes.entry(node.to_string()).or_insert_with(Vec::new).push(other.to_string());
        }
    }
    ...
    for k in nodes.keys() {
        ...
        node_indices.insert(k, idx);
    }

    for (k,v) in nodes.iter() {
        for o in v.iter() {
            graph.add_edge(node_indices[k], node_indices[o], ());
        }
    }

    // Extended https://github.com/Qiskit/rustworkx/blob/3bc7dc456eb611c9e7cc05122e8b58f4766add2a/rustworkx-core/src/connectivity/min_cut.rs#L121
    // to stop min-cut-search at cut-size 3
    let result = stoer_wagner_min_cut(&graph, |_| Ok::<i64, ()>(164), 3);
    let one_size = result.unwrap().unwrap().1.len();
    let other_size = node_indices.len() - one_size;
    let part2 =   one_size  * other_size;
    dbg!(&part2);
